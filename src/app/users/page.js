...
export default function UsersPage() {
  ...
  const handleSubmit = e => {
    ...
    action.then(res => {
      setUsers(u => editingId 
        ? u.map(x => x.id===editingId ? res.data : x)
        : [...u, res.data]
      )
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <UserList users={users} onEdit={setEditingId} onDelete={id => deleteUser(id).then(() => setUsers(u => u.filter(x=>x.id!==id)))} />
    </div>
  )
}
